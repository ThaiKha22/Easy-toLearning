...
export default function KeyConceptCard({ concept }) {
  ...
  const difficultyKey = String(concept.difficulty || '').toLowerCase();
  const difficultyLabel = t(`documentDetail.difficulty${difficultyKey.charAt(0).toUpperCase()}${difficultyKey.slice(1)}`, { defaultValue: concept.difficulty });
  return (
    <Card padding="p-5">
      <div className="flex items-start justify-between gap-2">
        <h3 className="font-display text-sm font-semibold text-ink-900">{concept.name}</h3>
        <Badge variant={diffVariant[concept.difficulty] || diffVariant[concept.difficulty?.charAt(0).toUpperCase() + concept.difficulty?.slice(1)]}>{difficultyLabel}</Badge>
      </div>
