  ...
  return (
    <Card padding="p-5">
      <div className="flex items-center justify-between">
        <h2 className="font-display text-base font-semibold text-ink-900">Today's Study Plan</h2>
        <Link to="/study-plan" className="text-sm font-medium text-brand-600 hover:underline">Full plan</Link>
      </div>
      <ul className="mt-4 space-y-2.5">
        {items.map((item) => (
          <li key={item.id} className="flex items-center gap-3 rounded-xl border border-line-soft px-3.5 py-2.5">
            ...
            <span className="text-xs text-ink-500">{item.duration} min</span>
